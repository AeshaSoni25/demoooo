...
export function TopNavbar({ onMenuClick, title }: TopNavbarProps) {
  ...
  const handleSearch = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      setSelectedLocation(val || "All Regions");
      if (val) {
        router.push(`?region=${encodeURIComponent(val)}`);
      } else {
        router.push("?");
      }
    }
  ...
  const handleSelectLocation = (loc: string) => {
    ...
    if (loc === "All Regions") {
      ...
      router.push(`?region=${encodeURIComponent(loc)}`);
    }
  };
    ...
    >
      <div className="flex items-center h-full px-4 gap-3">
        ...
        <div className="hidden md:flex flex-1 max-w-64 relative">
          ...
          <input
            type="search"
            placeholder="Search region (e.g. California) + Enter..."
