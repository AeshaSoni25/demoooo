...
export function generateStaticParams() {
  return DEMO_LOCATIONS.map((loc) => ({ id: loc.id }));
}

export default function LocationPage({ params }: PageProps) {
  const location = DEMO_LOCATIONS.find((l) => l.id === params.id);
  if (!location) notFound();

  const locationAlerts = DEMO_ALERTS.filter(
    (a) => a.locationId === location.id
  );
  const locationEvents = DEMO_HISTORICAL_EVENTS.filter(
    (e) => e.locationId === location.id
  );
  const locationSensors = DEMO_SENSORS.filter(
